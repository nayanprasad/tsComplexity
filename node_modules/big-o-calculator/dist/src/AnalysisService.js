...
class AnalysisService {
    ...
    analyze(code, runnerContext = {}) {
        let testSet = this.testSetCreator.create(code);
        let testResultsObj = { r: [] };
        let calculator = this.calculatorForLanguage(code.language);
    ...
    recursiveAnalyze(calculator, testSet, testResultsObj, i = 0, runnerContext = {}) {
        return this.runSample(testSet.code, testSet.samples[i], testResultsObj, runnerContext)
            .then((testResults) => {
            let bigO = calculator.calculate(testResults, testSet.code.expectedSpeed);
            ...
            if (testResults[testResults.length - 1].result >= calculator_1.TIMEOUT_THRESHOLD_MS) {
                let err = new runner_2.UnexpectedTimeoutError('Calculator could not handle runner timeout.');
    ...
    runSample(code, sample, testResultsObj, runnerContext = {}) {
        ...
        while (runs > 0) {
            runs--;
            let durationRunner = this.codeRunner.checkCodeExecDuration(code.language, sample.code, runnerContext)
                .then((duration) => {
                return Promise.resolve({
                    result: duration,
                    n: sample.n
    ...
    runCountForSample(language, sampleSize) {
        if (this.repeatedSamples.has(language)) {
            let samples = this.repeatedSamples.get(language);
